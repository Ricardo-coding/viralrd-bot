...
async function iniciarBot() {
  ...
  sock.ev.on("connection.update", (update) => {
    ...
    if (qr) {
      ...
      console.log("📱 Nuevo QR generado — entra a /qr para escanearlo");
    }

    if (connection === "close") {
      const motivo = lastDisconnect?.error?.output?.statusCode;
      const mensajeError = lastDisconnect?.error?.message || "sin detalle";
      const debeReconectar = motivo !== DisconnectReason.loggedOut;
      estadoConexion = `Conexión cerrada (código ${motivo}: ${mensajeError}), reconectando...`;
      console.log("Conexión cerrada. Código:", motivo, "Mensaje:", mensajeError, "Reconectar:", debeReconectar);
      if (debeReconectar) setTimeout(iniciarBot, 3000);
